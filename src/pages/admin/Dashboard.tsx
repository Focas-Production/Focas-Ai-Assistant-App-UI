import React, { useState } from 'react';
import logo from '../../assets/logo.png';
import adminIcon from '../../assets/admin.png';
import backarrow from '../../assets/backarrow.png';

const AdminSidebar: React.FC = () => (
  <div className="bg-[#120088] text-white w-60 h-screen flex flex-col justify-between fixed font-poppins">
    {/* Top Profile */}
    <div className="p-4 flex items-center gap-3">
      <img src={adminIcon} alt="Admin" className="w-10 h-10 rounded-full" />
      <div>
        <p className="text-[20px] font-semibold leading-5">karthik</p>
        <p className="text-sm text-gray-300">Admin</p>
      </div>
    </div>
    {/* Navigation Menu */}
    <nav className="flex flex-col gap-2 text-[16px] mt-[-600px]">
      <a href="/admin/dashboard" className="flex items-center gap-3 px-6 py-3 font-semibold bg-white text-[#120088] border-l-4 border-green-400">
        <span className="material-icons">group</span> Manage People
      </a>
      <a href="/admin/student-details" className="flex items-center gap-3 px-6 py-3 font-semibold text-white hover:bg-[#100070]">
        <span className="material-icons">groups</span> Students
      </a>
      <a href="/admin/tutor-details" className="flex items-center gap-3 px-6 py-3 font-semibold text-white hover:bg-[#100070]">
        <span className="material-icons">supervisor_account</span> Tutors
      </a>
      <a href="/admin/admin-details" className="flex items-center gap-3 px-6 py-3 font-semibold text-white hover:bg-[#100070]">
        <span className="material-icons">security</span> Admin
      </a>
    </nav>
    {/* Bottom Logout */}
    <div className="mb-6 pl-6">
      <button className="flex items-center gap-3 text-[16px] text-gray-300 hover:text-white">
        <span className="material-icons">logout</span> Log Out
      </button>
    </div>
  </div>
);

const AdminDashboard = () => {
  const [showModal, setShowModal] = useState(false);
  const [name, setName] = useState('');
  const [phone, setPhone] = useState('');
  const [role, setRole] = useState('');
  const [level, setLevel] = useState('');
  const people = [
    { name: 'AAA', phone: 'XXXX', role: 'Tutor' },
    { name: 'AAA', phone: 'XXXX', role: 'Student' },
  ];
  return (
    <div className="flex h-screen bg-[#f7f9fc]">
      <AdminSidebar />
      <div className="flex-1 ml-60 flex flex-col relative">
        {/* Header */}
        <div className="flex justify-end items-center h-20 px-10 border-b border-gray-200 bg-transparent">
          <img src={logo} alt="FOCAS Logo" className="h-10 w-auto" />
        </div>
        {/* Add Button */}
        <div className="flex justify-center mt-8">
          <button
            className="bg-[#120088] text-white px-16 py-3 rounded-xl font-semibold text-lg shadow-md hover:bg-[#100070] transition"
            onClick={() => setShowModal(true)}
          >
            Add +
          </button>
        </div>
        {/* Table */}
        <div className="flex-1 flex flex-col items-center justify-center px-4">
          <div className="w-full max-w-3xl mt-4">
            <table className="w-full border border-[#120088] text-sm rounded-lg overflow-hidden bg-white">
              <thead className="bg-[#f7f9fc] border-b border-[#120088]">
                <tr>
                  <th className="py-2 px-4 text-[#120088] text-lg font-bold">Name</th>
                  <th className="py-2 px-4 text-[#120088] text-lg font-bold">Phone Number</th>
                  <th className="py-2 px-4 text-[#120088] text-lg font-bold">Role</th>
                  <th className="py-2 px-4"></th>
                  <th className="py-2 px-4">
                    <button className="bg-green-500 text-white px-4 py-1 rounded font-semibold text-sm">Export-CSV</button>
                  </th>
                </tr>
              </thead>
              <tbody>
                {people.map((row, idx) => (
                  <tr key={idx} className={`border-b border-[#120088] last:border-b-0 ${idx === 1 ? 'bg-[#120088] text-white' : ''}`}>
                    <td className="py-2 px-4">{row.name}</td>
                    <td className="py-2 px-4">{row.phone}</td>
                    <td className="py-2 px-4">{row.role}</td>
                    <td className="py-2 px-4">
                      <button className={`px-6 py-2 rounded-lg font-semibold transition ${idx === 1 ? 'bg-white text-[#120088]' : 'bg-[#120088] text-white'}`}>View</button>
                    </td>
                    <td className="py-2 px-4">
                      <button className="text-[#120088] text-xl">&#8681;</button>
                    </td>
                  </tr>
                ))}
              </tbody>
            </table>
            {/* Pagination */}
            <div className="flex justify-center items-center gap-4 mt-6">
              <button className="w-8 h-8 flex items-center justify-center border-2 border-[#120088] rounded-full text-[#120088] text-lg hover:bg-[#120088] hover:text-white transition">&lt;</button>
              <button className="w-8 h-8 flex items-center justify-center border-2 border-[#120088] rounded-full text-[#120088] text-lg hover:bg-[#120088] hover:text-white transition">&gt;</button>
            </div>
          </div>
        </div>
        {/* Modal Popup */}
        {showModal && (
          <div className="fixed inset-0 bg-black bg-opacity-30 flex items-center justify-center z-20">
            <div className="bg-white rounded-xl shadow-2xl p-10 min-w-[500px] max-w-[520px] mx-4 flex flex-col items-center relative">
              {/* Back Button */}
              <button
                className="absolute left-4 top-4 w-8 h-8 flex items-center justify-center rounded-full border border-[#120088] text-[#120088] hover:bg-[#120088] hover:text-white transition"
                onClick={() => setShowModal(false)}
                aria-label="Back"
              >
                <img src={backarrow} alt="Back" className="w-5 h-5" />
              </button>
              <form className="w-full flex flex-col items-center gap-6 mt-4">
                <input
                  type="text"
                  className="w-full border-2 border-[#120088] rounded-lg px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-[#120088]"
                  placeholder="Enter the Name:"
                  value={name}
                  onChange={e => setName(e.target.value)}
                />
                <input
                  type="text"
                  className="w-full border-2 border-[#120088] rounded-lg px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-[#120088]"
                  placeholder="Enter the Phone number:"
                  value={phone}
                  onChange={e => setPhone(e.target.value)}
                />
                <div className="flex gap-6 w-full">
                  <div className="flex-1">
                    <select
                      className="w-full border-2 border-[#120088] rounded-lg px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-[#120088] appearance-none"
                      value={role}
                      onChange={e => setRole(e.target.value)}
                    >
                      <option value="" disabled>Role</option>
                      <option value="Admin">Admin</option>
                      <option value="Tutor">Tutor</option>
                      <option value="Student">Student</option>
                    </select>
                  </div>
                  <div className="flex-1">
                    <select
                      className="w-full border-2 border-[#120088] rounded-lg px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-[#120088] appearance-none"
                      value={level}
                      onChange={e => setLevel(e.target.value)}
                    >
                      <option value="" disabled>Level</option>
                      <option value="Foundation">Foundation</option>
                      <option value="Intermediate">Intermediate</option>
                      <option value="Final">Final</option>
                      <option value="Null">Null</option>
                    </select>
                  </div>
                </div>
                <button
                  type="submit"
                  className="w-full bg-green-500 hover:bg-green-600 text-white font-semibold py-3 rounded-lg text-lg mt-4 shadow-md transition"
                >
                  Submit
                </button>
              </form>
            </div>
          </div>
        )}
      </div>
    </div>
  );
};

export default AdminDashboard; 