...
import { type ReactElement, type JSXElementConstructor, type ReactNode, type ReactPortal, useMemo, useState } from 'react';

const TutorStudentDetails = () => {
  const [students] = useState([
    { id: 1, name: "Alice Johnson", studentId: "STU001", subject: "Mathematics", progress: 85, attendance: "92%", streak: 10, status: "Active", joinDate: "2023-09-15" },
    { id: 2, name: "Brian Smith", studentId: "STU002", subject: "Science", progress: 78, attendance: "88%", streak: 8, status: "Active", joinDate: "2024-01-20" },
    { id: 3, name: "Clara Wilson", studentId: "STU003", subject: "English", progress: 92, attendance: "96%", streak: 15, status: "Active", joinDate: "2022-09-10" },
    { id: 4, name: "David Lee", studentId: "STU004", subject: "History", progress: 81, attendance: "90%", streak: 9, status: "Inactive", joinDate: "2025-05-01" },
    { id: 5, name: "Eva Brown", studentId: "STU005", subject: "Mathematics", progress: 87, attendance: "94%", streak: 11, status: "Active", joinDate: "2023-09-15" },
  ]);
  ...
  const filteredAndSortedStudents = useMemo(() => {
    let filtered = students.filter((student: { name: string; studentId: string; subject: string; status: any; }) => {
  ...
  return (
    <div className="p-8 bg-gray-50 min-h-screen">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-4">
          <h2 className="text-3xl font-bold text-gray-900 mb-1">Tutor - Student Details</h2>
          <p className="text-gray-600 mb-4">Manage your students' sessions, progress, and engagement</p>
        </div>
        ...
        <div className="mb-8 bg-white rounded-lg shadow-sm p-6">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 text-center">
            ...
            <div>
              <div className="text-2xl font-bold text-orange-600">{new Set(students.map(s => s.subject)).size}</div>
              <div className="text-sm text-gray-500">Subjects</div>
            </div>
            <div>
              <div className="text-2xl font-bold text-purple-600">{(students.reduce((sum, s) => sum + s.progress, 0) / students.length).toFixed(0)}%</div>
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
                  {['studentId', 'name', 'subject', 'progress', 'attendance', 'streak', 'status'].map((key) => (
                    <th
                      key={key}
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {filteredAndSortedStudents.map((student) => (
                  <tr key={student.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-blue-600">{student.progress}%</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{student.attendance}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{student.streak}</td>
