...
import { apiService } from '../../services/api'; // Assuming your apiService is correctly located
import { Download, MessageCircle, FileText } from 'lucide-react';
import Navbar from '../../components/layout/Navbar';
import Sidebar from '../../components/layout/Sidebar';
...
const StudentReport = ({ showSidebar }: StudentReportProps) => {
  ...
  useEffect(() => {
    ...
    const fetchSessionData = async () => {
      ...
      try {
        ...
        setError("Could not load the session report. It might not exist or an error occurred.");
      } finally {
        setIsLoading(false);
      }
    };
  ...
  // --- UI AND USER LOGIC (Preserved from original code) ---
  useEffect(() => {
    const navigationSource = localStorage.getItem('reportNavigationSource');
    const userInfo = localStorage.getItem('userInfo');
  ...
  const calculateAccuracy = (messages: ChatMessage[]) => {
    ...
    userMessages.forEach((_, index) => {
      if (aiMessages[index]?.content) {
        respondedQuestions++;
        if (aiMessages[index].content.length > 100) { // Simple heuristic
          detailedResponses++;
        }
      }
    });
